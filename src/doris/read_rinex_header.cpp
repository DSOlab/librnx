...
int dso::DorisObsRinex::read_header() noexcept {
  ...
  if (!m_stream.is_open() || !m_stream.good()) return -1;

  if (m_stream.tellg()) m_stream.seekg(0);
  ...
  if (std::strncmp(line + 60, "RINEX VERSION / TYPE", 20)) return 10;
  ...
  m_stream.getline(line, MAX_HEADER_CHARS);
  if (std::strncmp(line + 60, "PGM / RUN BY / DATE", 19)) return 20;
  ...
  while (m_stream.getline(line, MAX_HEADER_CHARS) && !error) {
    if (!std::strncmp(line + 60, "SATELLITE NAME", 14)) {
      ...
      if (std::strncmp(line, "SPACEBORNE", 10)) error = 50;
       ...
       */
      tmp_sz = count_length_reverse(line, 19);
      if (tmp_sz < 1 || tmp_sz >= 19) error = 81;
      std::memcpy(antenna_number(), line, tmp_sz);
      antenna_number()[tmp_sz] = '\0';
      tmp_sz = count_length_reverse(line + 20, 19);
      if (tmp_sz < 1 || tmp_sz >= 19) error = 82;
      ...
      for (int code = 0; code < obs_types_num; code++) {
        ...
        try {
          /* may throw! */
          auto type(DorisObsType::char2obst(*s));
          int freq = 0;
       ...
       */
      if (std::strncmp(line + 48, "DOR", 3)) error = 121;
      try {
        m_time_of_first_obs =
      ...
      if (*line != 'D') error = 141;
      if (!m_obs_scale_factors.empty()) m_obs_scale_factors.clear();
      ...
      for (int code = 0; code < num_obs; code++) {
        ...
        try {
          ...
          if (it == m_obs_codes.cend()) error = 145;
      ...
      std::memcpy(refsta.m_station_code, line,
                  sizeof(refsta.m_station_code) - 1);
      ...
      if (auto it = std::find_if(m_stations.cbegin(), m_stations.cend(),
                                 [&refsta](const dso::doris_rnx::Beacon &s) {
                                   return !std::strcmp(s.code(), refsta.code());
                                 });
          it == m_stations.cend()) {
        error = 193;
        fprintf(stderr,
                "[ERROR] Failed matching 'TIME REF STATION' %s to list of "
                "beacons! (traceback: %s)\n",
    ...
    } else if (!std::strncmp(line + 60, "RCV CLOCK OFFS APPL", 19)) {
  ...
  /* check stream state */
  if ((!m_stream.good()) || (!m_end_of_head)) return -2;
